...
export function validateRegions(regions, steps) {
  ...
  for (const stage of regions) {
    ...
    for (const r of stage) {
      if (!Array.isArray(r) || r.length !== 4 || r.some(n => typeof n !== 'number' || !Number.isFinite(n))) fail(400, 'Invalid region coordinates.')
      const [x, y, w, h] = r
  ...
  return {
    ...
    save(payload) {
      const operation = queue.then(async () => {
        const { slideId, src, revision, regions, focusStyle, ratio } = payload
        const { steps } = await slide(slideId, src)
        ...
        all[slideId] = { src, ratio, focusStyle, regions }
