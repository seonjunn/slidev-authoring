...
const state = () => JSON.stringify({ regions:regions.value, focusStyle:style.value })
...
function restore(snapshot) {
  ...
  regions.value = value.regions; style.value = value.focusStyle; selected.value = -1
}
...
function move(event) {
  ...
  if (g.type === 'draw') boxes.value[g.index] = [Math.min(x, sx), Math.min(y, sy), Math.max(minimum, Math.abs(x - sx)), Math.max(minimum, Math.abs(y - sy))]
  else {
    const [a, b, w, h] = g.rect
    if (g.type === 'move') boxes.value[g.index] = [Math.max(-1.5, Math.min(101.5 - w, a + x - sx)), Math.max(-1.5 * props.ratio, Math.min(100 + 1.5 * props.ratio - h, b + y - sy)), w, h]
    else {
      ...
      boxes.value[g.index] = [left, top, right - left, bottom - top]
    }
  }
}
function end(event) {
...
async function save() {
  ...
  try {
    const payload = { slideId:props.slideId, src:props.src, revision:revision.value, ratio:props.ratio, focusStyle:style.value,
      regions:regions.value.map(stage => stage.map(r => r.map(n => Math.round(n*1000)/1000))) }
    ...
    <section ref="dialog" class="figure-editor" role="dialog" aria-modal="true" aria-labelledby="figure-editor-title" tabindex="-1" @keydown="keydown">
      ...
      <div class="editor-workspace">
        <div class="editor-canvas" :class="{drawing:mode==='draw' && !preview}">
          <svg ref="svg" :viewBox="`-1.5 -1.5 103 ${100 / ratio + 3}`" role="img" :aria-label="caption" @pointerdown="start($event)" @pointermove="move" @pointerup="end" @pointercancel="end">
            ...
            <g v-for="(r, i) in boxes" :key="i">
              <rect class="editor-highlight" :class="{frame:style==='frame'}" :x="r[0]" :y="r[1]/ratio" :width="r[2]" :height="r[3]/ratio" @pointerdown="start($event, i)" />
        ...
        <aside>
          ...
          <div class="region-list"><button v-for="(_, i) in boxes" :key="i" :class="{chosen:selected===i}" @click="selected=i; mode='select'; preview=false">영역 {{ i+1 }}</button></div>
...
.editor-highlight { fill:#7655ab;fill-opacity:.19;stroke:none;mix-blend-mode:multiply;rx:.45; }
.editor-highlight.frame { fill:none;stroke:#7655ab;stroke-width:2.2px;vector-effect:non-scaling-stroke; }
